...
import { Leave, LeaveCategory, Role, Staff, User, Department } from '@/src/models';
...
interface GetLeaveIntimationArgs {
  status?: string;
}
...
export const getLeaveIntimations = async (
  _: any,
  args: GetLeaveIntimationArgs,
  context: Context,
) => {
  try {
    ...
    const isHod = user?.roles?.some((role: any) => role?.name === 'HOD');
    if (!isHod) {
      throw new Error('Access denied.');
    }

    const hodStaff = await Staff.findOne({ where: { user_id: authUser.id } });
    if (!hodStaff) {
      ...
      include: [
        {
          ...
          as: 'staff',
          where: {
            department_id: hodStaff.department_id,
          },
    ...
    return leaves
      .filter((leave: any) => leave?.staff_id !== hodStaff.id)
      .map((leave: any) => {
    ...
    logger.error(`Error in getLeaveIntimations: ${error}`);
